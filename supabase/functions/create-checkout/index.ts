...
serve(async (req) => {
  ...
  try {
    ...
    const pricingMap: Record<string, { amount: number; name: string }> = {
      "Regular": { amount: 60000, name: "Regular Conference Registration" }, // $600
      "Student": { amount: 20000, name: "Student Conference Registration" } // $200
    };

    const selectedPlan = pricingMap[planType];
    if (!selectedPlan) {
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: { 
              ...
              description: `AI & UX Design Conference 2026 - ${planType} Registration`
            },
            unit_amount: selectedPlan.amount,
            recurring: { interval: "year" }, // Annual conference registration
          },
          quantity: 1,
        },
      ],
